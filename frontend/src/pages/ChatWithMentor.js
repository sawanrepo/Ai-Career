...
const ChatWithMentor = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const botMessage = { sender: 'mentor', message: res.data.message };
      setMessages(prev => [...prev, botMessage]);
    } catch (error) {
      console.error('Error:', error);
      const errorMessage = { sender: 'mentor', message: 'Something went wrong. Please try again later.' };
  ...
  const handleReset = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/tatva-chat-reset`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
